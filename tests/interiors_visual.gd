...
func run() -> void:
    for id in ["laundry","reservoir"]:
        ...
        if id=="laundry":
            ...
            scene.camera.position=Vector3(-1.45,2.65,-5.5)
            scene.heading=1.14;scene.target_heading=1.14
            scene.pitch=-.25;scene.target_pitch=-.25;scene._process(0.)
            var impact: Image = await capture(scene,"reservoir-pipe-impact")
            scene.water_material.set_shader_parameter("impact_strength",0.)
            var still_water: Image = await capture(scene,"reservoir-pipe-no-impact")
            assert(difference(impact,still_water)>.000005,"The outlet visibly disturbs the receiving water")
            scene.water_material.set_shader_parameter("impact_strength",1.)
            scene.time+=.27;scene._process(0.)
            await capture(scene,"reservoir-pipe-impact-later")
