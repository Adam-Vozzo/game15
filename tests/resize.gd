...
func run() -> void:
    for id in ["menu","coast","town","sea","main"]:
        ...
        for shape in [Vector2i(390,844),Vector2i(640,360),Vector2i(844,390),Vector2i(1280,800)]:
            ...
            if id=="menu":
                for i in range(4): assert(current_scene.cards[i].visible,"All four experiences must remain visible")
    ...
    print("PASS: live portrait, landscape and desktop resizing across the menu and all four scenes")
    quit(0)
