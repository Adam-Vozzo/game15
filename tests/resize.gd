...
func run() -> void:
    for id in ["menu","coast","town","main"]:
        change_scene_to_file("res://"+id+".tscn")
        await settle()
        for shape in [Vector2i(390,844),Vector2i(844,390),Vector2i(1280,800)]:
            root.size = shape
            await settle()
    ...
    print("PASS: live portrait, landscape and desktop resizing across the menu and all three scenes")
    quit(0)
