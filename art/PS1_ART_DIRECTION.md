...
- Geometry is spent on eave profiles, tile ridges, recessed glazing, latticework, balconies, gutters, catenary cables and readable prop silhouettes. Modular wall and roof pieces limit affine distortion. Terrain uses stable world coordinates to avoid distracting swimming on broad ground polygons.
- Tree branches combine angled two-triangle cards with original cedar sprays, broadleaf clusters and mountain-ash leaflets from `kasumi_foliage.aseprite`. Transparent cutouts provide silhouette detail without large solid polygon fans.
